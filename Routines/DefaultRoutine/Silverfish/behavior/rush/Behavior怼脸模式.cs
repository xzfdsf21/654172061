namespace HREngine.Bots
{
    public class BehaviorRush : Behavior
    {
        ...
        public override float getPlayfieldValue(Playfield p)
        {
            ...
            if (p.ownWeapon.Attack >= 1)
            {
                retval += p.ownWeapon.Attack * p.ownWeapon.Durability;
            }
            ...
            foreach (Minion m in p.ownMinions)
            {
                ...
                if (m.handcard.card.name == CardDB.cardName.silverhandrecruit && m.Attack == 1 && m.Hp == 1) retval -= 5;
            ...
            retval += p.anzOwnExtraAttackHp - p.anzEnemyExtraAttackHp / 2;
            //soulfire etc
