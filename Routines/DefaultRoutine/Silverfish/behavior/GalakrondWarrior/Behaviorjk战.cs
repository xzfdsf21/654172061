...
namespace HREngine.Bots
{
    ...
    public class BehaviorGalakrondWarrior : Behavior
    {
        ...
        public override float getPlayfieldValue(Playfield p)
        {
            ...
            if (p.ownWeapon.Attack > 0)
            {
                if (p.ownWeapon.Attack > 1) retval += p.ownWeapon.Attack * p.ownWeapon.Durability;
                else retval += p.ownWeapon.Attack * p.ownWeapon.Durability + 1;
            }

            if (!p.enemyHero.frozen)
            {
                retval -= p.enemyWeapon.Durability * p.enemyWeapon.Attack;
            }
            else
            {
            ...
            foreach (Minion m in p.ownMinions)
            {
                ...
                //if (m.handcard.card.isToken && m.Attack <= 2 && m.Hp <= 2) retval -= 5;
                //if (!penman.specialMinions.ContainsKey(m.name) && m.Attack <= 2 && m.Hp <= 2) retval -= 5;
            ...
            retval += p.anzOgOwnCThunAttackBonus;
            ...
            if (p.manaTurnEnd >= 2 && !useAbili && p.ownAbilityReady)
            {
                switch (p.ownHeroAblility.card.name)
                {
                    ...
                    case CardDB.cardName.daggermastery:
                        if (!(p.ownWeapon.Durability > 1 || p.ownWeapon.Attack > 1)) retval -= 10;
                        break;
